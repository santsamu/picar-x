...
def basic_face_tracking():
    ...
    with Picarx() as px:
        with PicarxLedController() as led:
            ...
            try:
                while True:
                    ...
                    face_detected = Vilib.detect_obj_parameter['human_n'] != 0
                    if face_detected and not face_led_on:
                        led.set_color((0, 255, 0))
                        face_led_on = True
                    elif not face_detected and face_led_on:
                        led.off()
                        face_led_on = False
